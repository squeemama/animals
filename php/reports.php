...
<div class="container">
    <div class="jumbotron backg2 p-3 p-md-5 text-white rounded">
        <div class="col-md-6 px-0">
          ...
          <a class="ml-3 mt-3 btn btn-warning btn-lg" target="_blank" href="http://www.utulok-piestany.sk/index.php?s=1&u=6&do=archive&lang" role="button"><?php echo $languages[$x]["reports_archive"] ?></a>
  ...
  <!-- the first form is just a bait, it won't be rendered on the page; no clue why...fuckin weird... --> 
  <form id="single_<?php echo $i ?>" action='report.php' method='get' ></form>
  <?php

  $i=2;
  foreach ($row2 as $row) {
    if($row['name_'.$x] !== "" && $row['name_'.$x]  !== NULL){
    if($i%2==0){ ?>
                ...
                <div class="row ">
                    ...
                    <div class="col-md-9">
                        <div class="card-block">
                            <h2 class="card-title"><?php echo $row['name_'.$x] ?></h2>
                            <p class="card-text text-justify text str"><?php echo substr($row['reports_descriptions_'.$x], 0, 250) ?>...</p>
                            <form id="single_<?php echo $i ?>" action='report.php' method='get' >
